...
export function Price({ className, ...props }: CardProps) {
  const min = useSelector(
    (state: { finance: { financeDetails: { minDownpayment: any } } }) =>
      state.finance.financeDetails.minDownpayment,
  );
  ...
  const initDownPayment = useSelector(
    (state: { finance: { financeDetails: { downPaymentPercent: any } } }) =>
      state.finance.financeDetails.downPaymentPercent,
  );
        ...
        {!!(price > 0) && (
          <div className="grid  max-w-sm items-center gap-1.5 mt-6">
            ...
            <Slider
              ...
              step={0.5}
              value={downPayment}
              onValueChange={(downPayment: React.SetStateAction<any[]>) =>
